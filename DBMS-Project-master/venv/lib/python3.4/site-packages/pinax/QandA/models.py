...
class Post(models.Model):
    ...
    author = models.ForeignKey(settings.AUTH_USER_MODEL, related_name="questions", verbose_name=_("Author"))
    ...
    primary_image = models.ForeignKey("Image", null=True, blank=True,
                                      related_name="add", verbose_name=_("Primary Image"))
    ...
    def rev(self, rev_id):
        return self.reviseq.get(pk=rev_id)

    def current(self):
        "the currently visible (latest published) revision"
        return self.reviseq.exclude(published=None).order_by("-published")[0]

    def latest(self):
        "the latest modified (even if not published) revision"
        try:
            return self.reviseq.order_by("-updated")[0]
        except IndexError:
            return None
    ...
    def sharable_url(self):
        ...
        if not self.is_published or self.is_future_published:
            if self.secret_key:
                return reverse("pinax_QandA:blog_post_secret", kwargs={"post_secret_key": self.secret_key})
            else:
    ...
    def get_absolute_url(self):
        if self.is_published:
            if settings.PINAX_BLOG_SLUG_UNIQUE:
                name = "pinax_QandA:blog_post_slug"
                kwargs = {
                    "post_slug": self.slug
                }
            else:
                name = "pinax_QandA:blog_post"
...
class Revision(models.Model):
    ...
    post = models.ForeignKey(Post, related_name="reviseq", verbose_name=_("Post"))
    ...
    author = models.ForeignKey(settings.AUTH_USER_MODEL, related_name="reviseq", verbose_name=_("Author"))
...
class Image(models.Model):
    ...
    post = models.ForeignKey(Post, related_name="image", verbose_name=_("Post"))
...
class ReviewComment(models.Model):
    ...
    post = models.ForeignKey(Post, related_name="review_comment", verbose_name=_("Post"))
